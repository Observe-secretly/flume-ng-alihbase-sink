...
/**
 * A simple sink which reads events from a channel and writes them to HBase. The Hbase configuration is picked up from
 * the first <tt>hbase-site.xml</tt> encountered in the classpath. This sink supports batch reading of events from the
 * channel, and writing them to Hbase, to minimize the number of flushes on the hbase tables. To use this sink, it has
 * to be configured with certain mandatory parameters:
 * <p>
 * <tt>table: </tt> The name of the table in Hbase to write to.
 * <p>
 * <tt>columnFamily: </tt> The column family in Hbase to write to.
 * <p>
 * This sink will commit each transaction if the table's write buffer size is reached or if the number of events in the
 * current transaction reaches the batch size, whichever comes first.
 * <p>
 * Other optional parameters are:
 * <p>
 * <tt>serializer:</tt> A class implementing {@link HbaseEventSerializer}. An instance of this class will be used to
 * write out events to hbase.
 * <p>
 * <tt>serializer.*:</tt> Passed in the configure() method to serializer as an object of
 * {@link org.apache.flume.Context}.
 * <p>
 * <tt>batchSize: </tt>This is the batch size used by the client. This is the maximum number of events the sink will
 * commit per transaction. The default batch size is 100 events.
 * <p>
 * <strong>Note: </strong> While this sink flushes all events in a transaction to HBase in one shot, Hbase does not
 * guarantee atomic commits on multiple rows. So if a subset of events in a batch are written to disk by Hbase and Hbase
 * fails, the flume transaction is rolled back, causing flume to write all the events in the transaction all over again,
 * which will cause duplicates. The serializer is expected to take care of the handling of duplicates etc. HBase also
 * does not support batch increments, so if multiple increments are returned by the serializer, then HBase failure will
 * cause them to be re-written, when HBase comes back up.
 */
public class HBaseSink extends AbstractSink implements Configurable {
    ...
    public void start() {
        ...
        try {
            if (HBaseSinkSecurityManager.isSecurityEnabled(config)) {
                hbaseUser = HBaseSinkSecurityManager.login(config, null, kerberosPrincipal, kerberosKeytab);
            }
        } catch (Exception ex) {
        ...
        try {
            ...
            System.out.println("初始化Hbase连接完成");
        ...
        System.out.println("校验列族信息");
        try {
            if (!runPrivileged(new PrivilegedExceptionAction<Boolean>() {

                @Override
                public Boolean run() throws IOException {
                    return table.getDescriptor().hasColumnFamily(columnFamily);
                }
            })) {
                throw new IOException("Table " + tableName + " has no such column family "
            ...
            sinkCounter.incrementConnectionFailedCount();
            throw new FlumeException("Error getting column family from HBase." + "Please verify that the table "
                                     + tableName + " and Column Family, " + Bytes.toString(columnFamily)
    ...
    public void configure(Context context) {
        ...
        String cf = context.getString(HBaseSinkConfigurationConstants.CONFIG_COLUMN_FAMILY);
        batchSize = context.getLong(HBaseSinkConfigurationConstants.CONFIG_BATCHSIZE, new Long(100));
        serializerContext = new Context();
        // If not specified, will use HBase defaults.
        eventSerializerType = context.getString(HBaseSinkConfigurationConstants.CONFIG_SERIALIZER);
        ...
        serializerContext.putAll(context.getSubProperties(HBaseSinkConfigurationConstants.CONFIG_SERIALIZER_PREFIX));
        columnFamily = cf.getBytes(Charsets.UTF_8);
        try {
            Class<? extends HbaseEventSerializer> clazz = (Class<? extends HbaseEventSerializer>) Class.forName(eventSerializerType);
        ...
        enableWal = context.getBoolean(HBaseSinkConfigurationConstants.CONFIG_ENABLE_WAL,
        ...
        batchIncrements = context.getBoolean(HBaseSinkConfigurationConstants.CONFIG_COALESCE_INCREMENTS,
        ...
        Integer port = null;
        /**
         * HBase allows multiple nodes in the quorum, but all need to use the same client port. So get the nodes in
         * host:port format, and ignore the ports for all nodes except the first one. If no port is specified, use
         * default.
         */
    ...
    private <T> T runPrivileged(final PrivilegedExceptionAction<T> action) throws Exception {
        if (hbaseUser != null) {
    ...
    /**
     * The method getFamilyMap() is no longer available in Hbase 0.96. We must use reflection to determine which version
     * we may use.
     */
    @VisibleForTesting
    static Method reflectLookupGetFamilyMap() {
        ...
        for (String methodName : methodNames) {
            try {
                ...
                logger.debug("Increment.{} does not exist. Exception follows.", methodName, e);
            } catch (SecurityException e) {
                logger.debug("No access to Increment.{}; Exception follows.", methodName, e);
            }
        }
        if (m == null) {
    ...
    private Map<byte[], NavigableMap<byte[], Long>> getFamilyMap(Increment inc) {
        Preconditions.checkNotNull(refGetFamilyMap, "Increment.getFamilymap() not found");
    ...
    /**
     * Perform "compression" on the given set of increments so that Flume sends the minimum possible number of RPC
     * operations to HBase per batch.
     * 
    ...
    /**
     * Helper function for {@link #coalesceIncrements} to increment a counter value in the passed data structure.
     * 
     ...
     */
    private void incrementCounter(Map<byte[], Map<byte[], NavigableMap<byte[], Long>>> counters, byte[] row,
